...
class Connect(object):
    ...
    def _connReadProxy(conn):
        ...
        if not kb.dnsMode and conn:
            headers = conn.info()
            if headers and hasattr(headers, "getheader") and (
                    headers.getheader(HTTP_HEADER.CONTENT_ENCODING, "").lower() in ("gzip", "deflate") \
    ...
    def getPage(**kwargs):
        ...
        if conf.offline:
            ...
            return getUnicode(randomStr(int(randomInt()), alphabet=[chr(_) for _ in xrange(256)]), {},
                              int(randomInt())), None, None
        ...
        try:
            ...
            if websocket_:
                ...
                ws.connect(url, header=("%s: %s" % _ for _ in headers.items() if _[0] not in ("Host",)),
                ...
                requestHeaders += "\n".join("%s: %s" % (
                getUnicode(key.capitalize() if isinstance(key, basestring) else key), getUnicode(value)) for
                ...
                if not kb.authHeader and getRequestHeader(req, HTTP_HEADER.AUTHORIZATION) and (
        ...
        except (urllib2.URLError, socket.error, socket.timeout, httplib.BadStatusLine, httplib.IncompleteRead,
                httplib.ResponseNotReady, struct.error, ProxyError, SqlmapCompressionException, WebSocketException), e:
            ...
            if "no host given" in tbMsg:
                ...
                if kb.testMode and kb.testType not in (None, PAYLOAD.TECHNIQUE.TIME, PAYLOAD.TECHNIQUE.STACKED):
                    singleTimeWarnMessage(
                        "there is a possibility that the target (or WAF) is dropping 'suspicious' requests")
            ...
            if not isinstance(page, unicode):
                if HTTP_HEADER.CONTENT_TYPE in (responseHeaders or {}) and not re.search(TEXT_CONTENT_TYPE_REGEX,
                                                                                         responseHeaders[
        ...
        if responseHeaders:
            logHeaders = "\n".join(
                    "%s: %s" % (getUnicode(key.capitalize() if isinstance(key, basestring) else key), getUnicode(value))
    ...
    def queryPage(value=None, place=None, content=False, getRatioValue=False, silent=False, method=None,
        ...
        if payload:
            ...
            if place == PLACE.CUSTOM_POST and kb.postHint:
                ...
                if place in (PLACE.GET, PLACE.URI, PLACE.COOKIE) and not conf.skipUrlEncode or place in (
                PLACE.POST, PLACE.CUSTOM_POST) and kb.postUrlEncode:
            ...
            if conf.hpp:
                ...
                if place in (PLACE.GET, PLACE.POST):
                    ...
                    if match:
                        ...
                        for splitter in (urlencode(','), ','):
                            payload = payload.replace(splitter,
        ...
        if conf.csrfToken:
            ...
            page, headers, code = Connect.getPage(url=conf.csrfUrl or conf.url,
                                                  data=conf.data if conf.csrfUrl == conf.url else None,
                                                  method=conf.method if conf.csrfUrl == conf.url else None,
                                                  cookie=conf.parameters.get(PLACE.COOKIE), direct=True, silent=True,
                                                  ua=conf.parameters.get(PLACE.USER_AGENT),
                                                  referer=conf.parameters.get(PLACE.REFERER),
                                                  host=conf.parameters.get(PLACE.HOST))
            match = re.search(r"<input[^>]+name=[\"']?%s[\"']?\s[^>]*value=(\"([^\"]+)|'([^']+)|([^ >]+))" % re.escape(
                conf.csrfToken), page or "")
                    ...
                    for _ in conf.cj:
                        if _.name == conf.csrfToken:
                            ...
                            if not any(conf.csrfToken in _ for _ in
                                ...
                                if post:
                                    post = "%s%s%s=%s" % (
                                    post, conf.paramDel or DEFAULT_GET_POST_DELIMITER, conf.csrfToken, token)
                                elif get:
                                    get = "%s%s%s=%s" % (
                                    get, conf.paramDel or DEFAULT_GET_POST_DELIMITER, conf.csrfToken, token)
                                else:
            ...
            for name, value in variables.items():
                if name != "__builtins__" and originals.get(name, "") != value:
                    if isinstance(value, (basestring, int)):
                        ...
                        regex = r"((\A|%s)%s=).+?(%s|\Z)" % (
                        re.escape(conf.cookieDel or DEFAULT_COOKIE_DELIMITER), name,
        ...
        if conf.safeFreq > 0:
            ...
            if kb.queryCounter % conf.safeFreq == 0:
                if conf.safeUrl:
                    Connect.getPage(url=conf.safeUrl, post=conf.safePost, cookie=cookie, direct=True, silent=True,
                                    ua=ua, referer=referer, host=host)
                elif kb.safeReq:
                    Connect.getPage(url=kb.safeReq.url, post=kb.safeReq.post, method=kb.safeReq.method,
        ...
        if kb.nullConnection and not content and not response and not timeBasedCompare:
            ...
            try:
                ...
                _, headers, code = Connect.getPage(url=uri, get=get, post=post, method=method, cookie=cookie, ua=ua,
                ...
                if headers:
                    ...
                    NULLCONNECTION.HEAD, NULLCONNECTION.SKIP_READ) and HTTP_HEADER.CONTENT_LENGTH in headers:
        ...
        if not pageLength:
            try:
                page, headers, code = Connect.getPage(url=uri, get=get, post=post, method=method, cookie=cookie, ua=ua,
                                                      referer=referer, host=host, silent=silent, auxHeaders=auxHeaders,
