...
class Web:
    ...
    def _webFileInject(self, fileContent, fileName, directory):
        ...
        uplQuery = getUnicode(fileContent).replace("WRITABLE_DIR", directory.replace('/', '\\\\') if Backend.isOs(
        ...
        for directory in directories:
            ...
            if self.webApi == WEB_API.ASP:
                ...
                if self.webUpload(backdoorName, backdoorDirectory,
                                  content=backdoorContent.replace("WRITABLE_DIR", backdoorDirectory).replace(
                                          "RUNCMD_EXE", _)):
