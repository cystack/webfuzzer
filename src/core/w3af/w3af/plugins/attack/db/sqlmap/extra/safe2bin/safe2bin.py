...
def safecharencode(value):
    ...
    if isinstance(value, basestring):
        if any(_ not in SAFE_CHARS for _ in value):
            ...
            y if (y in string.printable or isinstance(value, unicode) and ord(y) >= 160) else '\\x%02x' % ord(y)),
                            retVal, (unicode if isinstance(value, unicode) else str)())
...
def safechardecode(value, binary=False):
    ...
    if isinstance(value, basestring):
        ...
        while True:
            ...
            if match:
                retVal = retVal.replace(match.group("result"), (unichr if isinstance(value, unicode) else chr)(
            ...
            while True:
                ...
                if match:
                    retVal = retVal.replace(match.group("result"),
                                            chr(ord(binascii.unhexlify(match.group("result").lstrip("\\?")))))
                else:
                    break
