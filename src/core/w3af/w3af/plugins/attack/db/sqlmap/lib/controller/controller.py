...
def _formatInjection(inj):
    ...
    for stype, sdata in inj.data.items():
        ...
        data += "    Payload: %s\n" % urldecode(payload, unsafe="&",
                                                plusspace=(inj.place != PLACE.GET and kb.postSpaceToPlus))
...
def _saveToResultsFile():
    ...
    for key, value in results.items():
        place, parameter = key
        line = "%s,%s,%s,%s%s" % (safeCSValue(kb.originalUrls.get(conf.url) or conf.url), place, parameter,
                                  "".join(map(lambda x: techniques[x][0].upper(), sorted(value))), os.linesep)
        ...
        try:
            ...
            if conf.multipleTargets:
                ...
                if conf.forms:
                    ...
                    message = "URL %d:\n%s %s%s" % (hostCount, HTTPMETHOD.GET, targetUrl,
                                                    " (PageRank: %s)" % get_pagerank(
                                                        targetUrl) if conf.googleDork and conf.pageRank else "")
            ...
            if (len(kb.injections) == 0 or (len(kb.injections) == 1 and kb.injections[0].place is None)) \
                ...
                for place in parameters:
                    ...
                    skip |= (
                    place == PLACE.USER_AGENT and intersect(USER_AGENT_ALIASES, conf.skip, True) not in ([], None))
                    ...
                    for parameter, value in paramDict.items():
                        ...
                        elif conf.level < 4 and (parameter.upper() in IGNORE_PARAMETERS or parameter.upper().startswith(
