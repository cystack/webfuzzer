...
def _goInference(payload, expression, charsetType=None, firstChar=None, lastChar=None, dump=False, field=None):
    ...
    if not (timeBasedCompare and kb.dnsTest):
        if (conf.eta or conf.threads > 1) and Backend.getIdentifiedDbms() and not re.search("(COUNT|LTRIM)\(",
                                                                                            expression,
                                                                                            re.I) and not timeBasedCompare:
...
def _goInferenceProxy(expression, fromUser=False, batch=False, unpack=True, charsetType=None, firstChar=None,
    ...
    if fromUser and " FROM " in expression.upper() and ((Backend.getIdentifiedDbms() \
            ...
            Backend.getIdentifiedDbms() in FROM_DUMMY_TABLE and not \
        ...
        if conf.direct:
            ...
            if not conf.forceDns:
                ...
                if found and conf.dnsName:
                    _ = "".join(filter(None, (key if isTechniqueAvailable(value) else None for key, value in
