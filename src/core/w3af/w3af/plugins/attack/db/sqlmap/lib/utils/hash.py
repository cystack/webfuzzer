            ...
            for hash_ in hashes:
                ...
                if re.match(hash_regex, hash_):
                    ...
                    if hash_regex in (HASH.MYSQL, HASH.MYSQL_OLD, HASH.MD5_GENERIC, HASH.SHA1_GENERIC):
                        ...
                        item = [(user, hash_),
                                {'salt': hash_[4:12], 'count': 1 << ITOA64.index(hash_[3]), 'prefix': hash_[:12]}]
                ...
                try:
                    if _multiprocessing:
                        ...
                        for i in xrange(_multiprocessing.cpu_count()):
                            p = _multiprocessing.Process(target=_bruteProcessVariantA, args=(
                            attack_info, hash_regex, suffix, retVal, i, count, kb.wordlists, custom_wordlist))
                            processes.append(p)
                ...
                for suffix in suffix_list:
                    ...
                    try:
                        if _multiprocessing:
                            ...
                            for i in xrange(_multiprocessing.cpu_count()):
                                p = _multiprocessing.Process(target=_bruteProcessVariantB, args=(
