...
def _findUnionCharCount(comment, place, parameter, value, prefix, suffix, where=PAYLOAD.WHERE.ORIGINAL):
    ...
    def _orderByTechnique():
        def _orderByTest(cols):
            ...
            return not re.search(r"(warning|error|order by|failed)", page or "", re.I) and comparison(page,
                                                                                                      headers) or re.search(
                r"data types cannot be compared or sorted", page or "", re.I)
    ...
    try:
        ...
        if not isNullValue(kb.uChar):
            for regex in (kb.uChar, r'>\s*%s\s*<' % kb.uChar):
                contains = [(count, re.search(regex, page or "", re.IGNORECASE) is not None) for count, page in
                            pages.items()]
    ...
    for position in positions:
        ...
        if content and phrase in content:
            ...
            if where == PAYLOAD.WHERE.ORIGINAL:
                ...
                query = agent.forgeUnionQuery(randQueryUnescaped, position, count, comment, prefix, suffix, kb.uChar,
                                              where, multipleUnions=randQueryUnescaped2)
                payload = agent.payload(place=place, parameter=parameter, newValue=query, where=where)
                ...
                if not all(_ in content for _ in (phrase, phrase2)):
                    ...
                    _, FROM_DUMMY_TABLE.get(Backend.getIdentifiedDbms(), ""), " AS %s" % randomStr() if _ == 0 else "")
                                                                     for _ in xrange(LIMITED_ROWS_TEST_NUMBER)),
                                                      randomStr())
                    ...
                    content = "%s%s".lower() % (removeReflectiveValues(page, payload) or "", \
                                                removeReflectiveValues(
                                                    listToStrValue(headers.headers if headers else None), \
                                                    payload, True) or "")
...
def _unionTestByCharBruteforce(comment, place, parameter, value, prefix, suffix):
    ...
    if conf.uColsStop == conf.uColsStart:
        ...
        count = _findUnionCharCount(comment, place, parameter, value, prefix, suffix,
