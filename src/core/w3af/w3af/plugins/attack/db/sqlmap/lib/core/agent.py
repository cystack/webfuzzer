...
class Agent(object):
    ...
    def payload(self, place=None, parameter=None, value=None, newValue=None, where=None):
        ...
        if place == PLACE.URI:
            ...
            if kb.postHint in (POST_HINT.SOAP, POST_HINT.XML):
                ...
                origValue = extractRegexResult(r"(?s)\"\s*:\s*(?P<result>\d+\Z)", origValue) or extractRegexResult(
                    r'(?s)\s*(?P<result>[^"\[,]+\Z)', origValue)
            else:
        ...
        if place in (PLACE.URI, PLACE.CUSTOM_POST, PLACE.CUSTOM_HEADER):
            ...
            retVal = retVal.replace(CUSTOM_INJECTION_MARK_CHAR, "").replace(REPLACEMENT_MARKER,
            ...
            def _(pattern, repl, string):
                ...
                if match:
                    while True:
                        ...
                        if _:
                            try:
                                repl = repl.replace(_.group(0), match.group(
            ...
            if origValue:
                ...
                retVal = _(regex, "%s=%s" % (parameter, self.addPayloadDelimiters(newValue.replace("\\", "\\\\"))),
                           paramString)
            else:
                retVal = _(r"(\A|\b)%s=%s(\Z|%s|%s|\s)" % (
            ...
            if retVal == paramString and urlencode(parameter) != parameter:
                retVal = _(r"(\A|\b)%s=%s" % (re.escape(urlencode(parameter)), re.escape(origValue)),
                           "%s=%s" % (urlencode(parameter), self.addPayloadDelimiters(newValue.replace("\\", "\\\\"))),
                           paramString)
    ...
    def prefixQuery(self, expression, prefix=None, where=None, clause=None):
        ...
        # In any other case prepend with the full prefix
            ...
            if not (expression and expression[0] == ';') and not (
    ...
    def cleanupPayload(self, payload, origValue=None):
        ...
        if origValue is not None:
            payload = payload.replace("[ORIGVALUE]",
                                      origValue if origValue.isdigit() else unescaper.escape("'%s'" % origValue))
    ...
    def nullCastConcatFields(self, fields):
        ...
        if fields.startswith("(CASE") or fields.startswith("(IIF") or fields.startswith("SUBSTR") or fields.startswith(
    ...
    def getFields(self, query):
        ...
        # Function
        if re.search("\A\w+\(.*\)", fieldsToCastStr, re.I) or (
    ...
    def concatQuery(self, query, unpack=True):
        ...
        if unpack:
            concatenatedQuery = ""
            query = query.replace(", ", ',')
            fieldsSelectFrom, fieldsSelect, fieldsNoSelect, fieldsSelectTop, fieldsSelectCase, _, fieldsToCastStr, fieldsExists = self.getFields(
                query)
        ...
        DBMS.PGSQL, DBMS.ORACLE, DBMS.SQLITE, DBMS.DB2, DBMS.FIREBIRD, DBMS.HSQLDB):
            if fieldsExists:
            ...
            if fieldsExists:
                ...
                concatenatedQuery = concatenatedQuery.replace("SELECT TOP %s " % topNum,
    ...
    def forgeUnionQuery(self, query, position, count, comment, prefix, suffix, char, where, multipleUnions=None,
        ...
        if Backend.getIdentifiedDbms() in (DBMS.MYSQL, DBMS.PGSQL, DBMS.SQLITE):
            ...
            if forgeNotIn:
                ...
                if " ORDER BY " not in fromFrom:
                    ...
                    limitedQuery += "%s %s ORDER BY %s) ORDER BY %s" % (
    ...
    def forgeCaseStatement(self, expression):
        ...
        if Backend.getIdentifiedDbms() is not None:
            ...
            if "(IIF" not in caseExpression and Backend.getIdentifiedDbms() in FROM_DUMMY_TABLE and not caseExpression.upper().endswith(
