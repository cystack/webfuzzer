        ...
        try:
            while True:
                ...
                if field:
                    ...
                    if any(Backend.isDbms(dbms) for dbms in (DBMS.MYSQL, DBMS.MSSQL)) and not any(_ in field for _ in (
                    "COUNT", "CASE")):  # skip chunking of scalar expression (unneeded)
                ...
                if page and conf.noEscape:
                    page = re.sub(r"('|\%%27)%s('|\%%27).*?('|\%%27)%s('|\%%27)" % (kb.chars.start, kb.chars.stop), "",
                                  page)
                ...
                output = reduce(lambda x, y: x if x is not None else y, ( \
                    ...
                    extractRegexResult(check, listToStrValue(
                        [headers[header] for header in headers if header.lower() != HTTP_HEADER.URI.lower()] \
                ...
                if output is not None:
                    ...
                    trimmed = extractRegexResult(trimcheck, page, re.DOTALL | re.IGNORECASE) \
                              or extractRegexResult(trimcheck, listToStrValue(
                        [headers[header] for header in headers if header.lower() != HTTP_HEADER.URI.lower()] \
                            if headers else None), re.DOTALL | re.IGNORECASE) \
    ...
    for field in expressionFieldsList:
        ...
        if not suppressOutput:
            if kb.fileReadMode and output and output.strip():
                print
            elif output is not None and not (threadData.resumed and kb.suppressResumeInfo) and not (
                emptyFields and field in emptyFields):
...
def _errorReplaceChars(value):
    ...
    if value:
        retVal = retVal.replace(kb.chars.space, " ").replace(kb.chars.dollar, "$").replace(kb.chars.at, "@").replace(
            kb.chars.hash_, "#")
...
def errorUse(expression, dump=False):
    ...
    if (dump and (conf.limitStart or conf.limitStop)) or (" FROM " in \
                                                                  expression.upper() and (
        (Backend.getIdentifiedDbms() not in FROM_DUMMY_TABLE) \
            ...
            expression.upper().endswith(FROM_DUMMY_TABLE[Backend.getIdentifiedDbms()]))) \
                                                                  and (
                "(CASE" not in expression.upper() or ("(CASE" in expression.upper() and "WHEN use" in expression))) \
                ...
                def errorThread():
                    ...
                    while kb.threadContinue:
                        ...
                        with kb.locks.value:
                            ...
                            while threadData.shared.buffered and threadData.shared.lastFlushed + 1 == \
