...
def _setRequestParams():
            ...
            if re.search(JSON_RECOGNITION_REGEX, conf.data):
                ...
                if test and test[0] in ("q", "Q"):
                    ...
                    conf.data = re.sub(r'("(?P<name>[^"]+)"\s*:\s*"[^"]+)"',
                                       functools.partial(process, repl=r'\g<1>%s"' % CUSTOM_INJECTION_MARK_CHAR),
                                       conf.data)
                    conf.data = re.sub(r'("(?P<name>[^"]+)"\s*:\s*)(-?\d[\d\.]*\b)',
                                       functools.partial(process, repl=r'\g<0>%s' % CUSTOM_INJECTION_MARK_CHAR),
                                       conf.data)
                ...
                if test and test[0] in ("q", "Q"):
                    ...
                    conf.data = conf.data.replace(CUSTOM_INJECTION_MARK_CHAR, ASTERISK_MARKER)
                    conf.data = re.sub(r"(=[^%s]+)" % DEFAULT_GET_POST_DELIMITER,
                ...
                if test and test[0] in ("q", "Q"):
                    ...
                    conf.data = conf.data.replace(CUSTOM_INJECTION_MARK_CHAR, ASTERISK_MARKER)
                    conf.data = re.sub(r"(<(?P<name>[^>]+)( [^<]*)?>)([^<]+)(</\2)", functools.partial(process,
                                                                                                       repl=r"\g<1>\g<4>%s\g<5>" % CUSTOM_INJECTION_MARK_CHAR),
                                       conf.data)
    ...
    PLACE.GET, PLACE.POST)) and not kb.postHint and not CUSTOM_INJECTION_MARK_CHAR in (
            ...
            if not kb.processUserMarks:
                ...
                if place == PLACE.CUSTOM_HEADER:
                        ...
                        if CUSTOM_INJECTION_MARK_CHAR in re.sub(PROBLEMATIC_CUSTOM_INJECTION_PATTERNS, "", value):
                            ...
                            for i in xrange(len(parts) - 1):
                                conf.paramDict[place][
                                    "%s #%d%s" % (header, i + 1, CUSTOM_INJECTION_MARK_CHAR)] = "%s,%s" % (header,
                                                                                                           "".join(
                                                                                                                   "%s%s" % (
                                                                                                                   parts[
                                                                                                                       j],
                    ...
                    for i in xrange(len(parts) - 1):
                        ...
                        if name is None:
                            name = "%s#%s%s" % (
                            ("%s " % kb.postHint) if kb.postHint else "", i + 1, CUSTOM_INJECTION_MARK_CHAR)
                        conf.paramDict[place][name] = "".join(
                                "%s%s" % (parts[j], CUSTOM_INJECTION_MARK_CHAR if i == j else "") for j in
                                xrange(len(parts)))
    ...
    if conf.csrfToken:
        ...
        conf.paramDict.get(PLACE.GET, {}), conf.paramDict.get(PLACE.POST, {}))) and not conf.csrfToken in set(
                _[0].lower() for _ in conf.httpHeaders) and not conf.csrfToken in conf.paramDict.get(PLACE.COOKIE, {}):
