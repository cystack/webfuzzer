...
def bisection(payload, expression, length=None, charsetType=None, firstChar=None, lastChar=None, dump=False):
    ...
    try:
        ...
        if partialValue:
            ...
            isinstance(conf.firstChar, int) or (isinstance(conf.firstChar, basestring) and conf.firstChar.isdigit())):
        ...
        if "LENGTH(" in expression.upper() or "LEN(" in expression.upper():
            ...
            isinstance(conf.lastChar, int) or (isinstance(conf.lastChar, basestring) and conf.lastChar.isdigit())):
        ...
        def validateChar(idx, value):
            ...
            if "'%s'" % CHAR_INFERENCE_MARK not in payload:
                forgedPayload = safeStringFormat(payload.replace(INFERENCE_GREATER_CHAR, INFERENCE_NOT_EQUALS_CHAR),
                ...
                forgedPayload = safeStringFormat(payload.replace(INFERENCE_GREATER_CHAR, INFERENCE_NOT_EQUALS_CHAR),
                                                 (expressionUnescaped, idx)).replace(markingValue, unescapedCharValue)
                        ...
                        if kb.threadContinue:
                            if showEta:
                                ...
                                if conf.verbose in (1, 2) and not showEta and not hasattr(conf, "api"):
                                    ...
                                    time.strftime("%X"), filterControlChars(output)))
        ...
        # No multi-threading (--threads = 1)
            ...
            while True:
                ...
                if conf.predictOutput and len(partialValue) > 0 and kb.partRun is not None:
                    ...
                    if commonValue is not None:
                        ...
                        query = agent.prefixQuery(
                            query.replace("[INFERENCE]", "(%s)=%s" % (expressionUnescaped, testValue)))
                ...
                if len(partialValue) > INFERENCE_BLANK_BREAK and partialValue[
                                                                 -INFERENCE_BLANK_BREAK:].isspace() and partialValue.strip(
                        ' ')[-1:] != '\n':
