...
def _oneShotUnionUse(expression, unpack=True, limited=False):
    retVal = hashDBRetrieve("%s%s" % (conf.hexConvert, expression),
    ...
    if retVal is None:
        ...
        query = agent.forgeUnionQuery(injExpression, vector[0], vector[1], vector[2], vector[3], vector[4], vector[5],
                                      vector[6], None, limited)
        ...
        def _(regex):
            return reduce(lambda x, y: x if x is not None else y, ( \
                ...
                extractRegexResult(regex, removeReflectiveValues(listToStrValue(headers.headers \
                                                                                    if headers else None), payload,
                                                                 True), re.DOTALL | re.IGNORECASE)), \
                          None)
                        ...
                        if output:
                            with kb.locks.value:
                                if all(map(lambda _: _ in output, (kb.chars.start, kb.chars.stop))):
                                    ...
                                    if threadData.shared.showEta:
                                        threadData.shared.progress.progress(time.time() - valueStart,
                            ...
                            if conf.verbose == 1 and not (
                                ...
                                status = "[%s] [INFO] %s: %s" % (
                                time.strftime("%X"), "resumed" if threadData.resumed else "retrieved", safecharencode(
                                    ",".join(
                                            "\"%s\"" % _ for _ in flattenValue(arrayizeValue(items))) if not isinstance(
